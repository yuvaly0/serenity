...
DurationFormatConstructor::DurationFormatConstructor(Realm& realm)
    : NativeFunction(realm.vm().names.DurationFormat.as_deprecated_string(), realm.intrinsics().function_prototype())
{
}
...
ThrowCompletionOr<NonnullGCPtr<Object>> DurationFormatConstructor::construct(FunctionObject& new_target)
{
    ...
    opt.nu = numbering_system.is_undefined() ? Optional<String>() : numbering_system.as_deprecated_string().utf8_string();
    ...
    duration_format->set_style(style.as_deprecated_string().utf8_string_view());
    ...
    for (auto const& duration_instances_component : duration_instances_components) {
        ...
        auto unit_options = TRY(get_duration_unit_options(vm, unit, *options, style.as_deprecated_string().utf8_string_view(), value_list, digital_base, previous_style));
