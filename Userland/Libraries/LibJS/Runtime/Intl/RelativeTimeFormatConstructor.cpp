...
RelativeTimeFormatConstructor::RelativeTimeFormatConstructor(Realm& realm)
    : NativeFunction(realm.vm().names.RelativeTimeFormat.as_deprecated_string(), realm.intrinsics().function_prototype())
{
}
...
ThrowCompletionOr<NonnullGCPtr<RelativeTimeFormat>> initialize_relative_time_format(VM& vm, RelativeTimeFormat& relative_time_format, Value locales_value, Value options_value)
{
    ...
    relative_time_format.set_style(style.as_deprecated_string().utf8_string_view());
    ...
    relative_time_format.set_numeric(numeric.as_deprecated_string().utf8_string_view());
