...
NumberFormatConstructor::NumberFormatConstructor(Realm& realm)
    : NativeFunction(realm.vm().names.NumberFormat.as_deprecated_string(), realm.intrinsics().function_prototype())
{
}
...
ThrowCompletionOr<NonnullGCPtr<NumberFormat>> initialize_number_format(VM& vm, NumberFormat& number_format, Value locales_value, Value options_value)
{
    ...
    if (!numbering_system.is_undefined()) {
        ...
        if (!::Locale::is_type_identifier(numbering_system.as_deprecated_string().utf8_string_view()))
            return vm.throw_completion<RangeError>(ErrorType::OptionIsNotValidValue, numbering_system, "numberingSystem"sv);

        // 8. Set opt.[[nu]] to numberingSystem.
        opt.nu = numbering_system.as_deprecated_string().utf8_string();
    }
    ...
    number_format.set_notation(notation.as_deprecated_string().utf8_string_view());
    ...
    if (number_format.notation() == NumberFormat::Notation::Compact) {
        ...
        number_format.set_compact_display(compact_display.as_deprecated_string().utf8_string_view());
    ...
    number_format.set_sign_display(sign_display.as_deprecated_string().utf8_string_view());
...
ThrowCompletionOr<void> set_number_format_digit_options(VM& vm, NumberFormatBase& intl_object, Object const& options, int default_min_fraction_digits, int default_max_fraction_digits, NumberFormat::Notation notation)
{
    ...
    auto rounding_priority = rounding_priority_option.as_deprecated_string().utf8_string_view();
    ...
    intl_object.set_rounding_mode(rounding_mode.as_deprecated_string().utf8_string_view());

    // 16. Set intlObj.[[TrailingZeroDisplay]] to trailingZeroDisplay.
    intl_object.set_trailing_zero_display(trailing_zero_display.as_deprecated_string().utf8_string_view());
...
ThrowCompletionOr<void> set_number_format_unit_options(VM& vm, NumberFormat& intl_object, Object const& options)
{
    ...
    intl_object.set_style(style.as_deprecated_string().utf8_string_view());
    ...
    else if (!is_well_formed_currency_code(currency.as_deprecated_string().utf8_string_view()))
    ...
    else if (!is_well_formed_unit_identifier(unit.as_deprecated_string().utf8_string_view()))
    ...
    if (intl_object.style() == NumberFormat::Style::Currency) {
        ...
        intl_object.set_currency(MUST(currency.as_deprecated_string().utf8_string().to_uppercase()));

        // c. Set intlObj.[[CurrencyDisplay]] to currencyDisplay.
        intl_object.set_currency_display(currency_display.as_deprecated_string().utf8_string_view());

        // d. Set intlObj.[[CurrencySign]] to currencySign.
        intl_object.set_currency_sign(currency_sign.as_deprecated_string().utf8_string_view());
    }

    // 15. If style is "unit", then
    if (intl_object.style() == NumberFormat::Style::Unit) {
        // a. Set intlObj.[[Unit]] to unit.
        intl_object.set_unit(unit.as_deprecated_string().utf8_string());

        // b. Set intlObj.[[UnitDisplay]] to unitDisplay.
        intl_object.set_unit_display(unit_display.as_deprecated_string().utf8_string_view());
    }
