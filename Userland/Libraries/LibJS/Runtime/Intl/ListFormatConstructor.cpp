...
ListFormatConstructor::ListFormatConstructor(Realm& realm)
    : NativeFunction(realm.vm().names.ListFormat.as_deprecated_string(), realm.intrinsics().function_prototype())
{
}
...
ThrowCompletionOr<NonnullGCPtr<Object>> ListFormatConstructor::construct(FunctionObject& new_target)
{
    ...
    list_format->set_type(type.as_deprecated_string().utf8_string_view());
    ...
    list_format->set_style(style.as_deprecated_string().utf8_string_view());
