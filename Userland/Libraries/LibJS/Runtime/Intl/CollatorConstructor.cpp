...
static ThrowCompletionOr<NonnullGCPtr<Collator>> initialize_collator(VM& vm, Collator& collator, Value locales_value, Value options_value)
{
    ...
    collator.set_usage(usage.as_deprecated_string().utf8_string_view());
    ...
    if (!collation.is_undefined()) {
        ...
        if (!::Locale::is_type_identifier(collation.as_deprecated_string().utf8_string_view()))
            return vm.throw_completion<RangeError>(ErrorType::OptionIsNotValidValue, collation, "collation"sv);

        // 12. Set opt.[[co]] to collation.
        opt.co = collation.as_deprecated_string().utf8_string();
    }
    ...
    if (!case_first.is_undefined())
        opt.kf = case_first.as_deprecated_string().utf8_string();
    ...
    collator.set_sensitivity(sensitivity.as_deprecated_string().utf8_string_view());
...
CollatorConstructor::CollatorConstructor(Realm& realm)
    : NativeFunction(realm.vm().names.Collator.as_deprecated_string(), realm.intrinsics().function_prototype())
{
}
