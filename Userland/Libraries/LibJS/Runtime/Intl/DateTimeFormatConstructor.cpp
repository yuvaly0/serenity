...
DateTimeFormatConstructor::DateTimeFormatConstructor(Realm& realm)
    : NativeFunction(realm.vm().names.DateTimeFormat.as_deprecated_string(), realm.intrinsics().function_prototype())
{
}
...
ThrowCompletionOr<NonnullGCPtr<DateTimeFormat>> create_date_time_format(VM& vm, FunctionObject& new_target, Value locales_value, Value options_value, OptionRequired required, OptionDefaults defaults)
{
    ...
    if (!calendar.is_undefined()) {
        ...
        if (!::Locale::is_type_identifier(calendar.as_deprecated_string().utf8_string_view()))
            return vm.throw_completion<RangeError>(ErrorType::OptionIsNotValidValue, calendar, "calendar"sv);

        // 9. Set opt.[[ca]] to calendar.
        opt.ca = calendar.as_deprecated_string().utf8_string();
    }
    ...
    if (!hour_cycle.is_nullish())
        opt.hc = hour_cycle.as_deprecated_string().utf8_string();
    ...
    TRY(for_each_calendar_field(vm, format_options, [&](auto& option, auto const& property, auto const& values) -> ThrowCompletionOr<void> {
        ...
        // c. Else,
            ...
            if (!value.is_undefined()) {
                option = ::Locale::calendar_pattern_style_from_string(value.as_deprecated_string().utf8_string_view());
    ...
    if (!date_style.is_undefined())
        date_time_format->set_date_style(date_style.as_deprecated_string().utf8_string_view());
    ...
    if (!time_style.is_undefined())
        date_time_format->set_time_style(time_style.as_deprecated_string().utf8_string_view());
    ...
    // 44. Else,
        ...
        if (matcher.as_deprecated_string().utf8_string_view() == "basic"sv) {
