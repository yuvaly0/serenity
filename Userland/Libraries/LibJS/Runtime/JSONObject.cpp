...
ThrowCompletionOr<DeprecatedString> JSONObject::stringify_impl(VM& vm, Value value, Value replacer, Value space)
{
            ...
            if (is_array) {
                ...
                for (size_t i = 0; i < replacer_length; ++i) {
                    ...
                    if (replacer_value.is_string()) {
                        item = replacer_value.as_deprecated_string().deprecated_string();
    ...
    if (space.is_number()) {
        ...
        auto string = space.as_deprecated_string().deprecated_string();
        if (string.length() <= 10)
            state.gap = string;
        else
...
ThrowCompletionOr<DeprecatedString> JSONObject::serialize_json_property(VM& vm, StringifyState& state, PropertyKey const& key, Object* holder)
{
    ...
    if (value.is_string())
        return quote_json_string(value.as_deprecated_string().deprecated_string());
...
ThrowCompletionOr<DeprecatedString> JSONObject::serialize_json_object(VM& vm, StringifyState& state, Object& object)
{
    ...
    if (state.property_list.has_value()) {
        ...
        for (auto& property : property_list)
            TRY(process_property(property.as_deprecated_string().deprecated_string()));
    }
    ...
    if (value.is_object()) {
        ...
        if (is_array) {
            ...
            for (auto& property_key : property_list)
                TRY(process_property(property_key.as_deprecated_string().deprecated_string()));
        }
    }
