...
GUI::Model::MatchResult ManualModel::data_matches(const GUI::ModelIndex& index, const GUI::Variant& term) const
{
    ...
    auto match_result = fuzzy_match(term.as_deprecated_string(), name.value());
    ...
    if (view_result.value().contains(term.as_deprecated_string(), CaseSensitivity::CaseInsensitive))
